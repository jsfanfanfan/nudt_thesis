...
\begin{itemize}
	\item \textbf{uniform}（均匀分块）：将当前流水级内的所有层均匀分成多个 chunk，每个 chunk 包含 \texttt{recompute\_num\_layers} 层；每个 chunk 的输入被 checkpoint，chunk 内前向一次执行，反向时整块先重计算再反传。
...
各子模块在各自的 \texttt{forward} 中根据配置项\texttt{recompute\_modules} 决定舍弃哪些子模块的中间激活值，在反向传播时哪些模块的激活值需要重新计算。

选择性子模块方面，\texttt{recompute\_modules} 指定在 selective 粒度下要对哪些子模块做重计算。
不同子模块采用的 checkpoint 机制不同：\emph{标准 checkpoint} 保存输入、反向时重新前向再反传；
\emph{Output-discarding checkpoint} 在前向完成后丢弃该子模块输出的显存，在反向时通过梯度 hook 再算一遍并写回，可进一步省显存。
表~\ref{tab:chap4-megatron-modules} 列出常用选项、含义及 checkpoint 类型。
...
若未指定 \texttt{recompute\_modules}，则默认为 \texttt{["core\_attn"]}。稠密模型下常用的“最省显存”组合为 \texttt{[core\_attn, mlp, layernorm]}。部分子模块有前置条件：例如 \texttt{moe\_act} 需与 \texttt{moe\_grouped\_gemm} 同时开启，\texttt{mla\_up\_proj} 仅在启用 \texttt{multi\_latent\_attention} 时有效；CPU offloading 与任意重计算互斥。

上述策略在实现上对\emph{同一训练任务内的所有流水级、所有设备}采用统一的 \texttt{recompute\_granularity}、\texttt{recompute\_method}、\texttt{recompute\_num\_layers} 与 \texttt{recompute\_modules}，即面向同构集群的“全局一致”策略。
在异构集群上，显存紧张流水级若与显存充裕流水级使用相同配置，易出现前者仍 OOM 或后者重算过多的问题，
而且目前的重计算技术给所有流水级增加了计算负载，这使得异构 GPU 集群中性能较差的 GPU 进一步成为计算瓶颈
为下文针对各流水级/设备做差异化配置的异构重计算提供了动机。

\subsection{异构重计算策略设计}
\label{sec:HR-MMoH-design}

在 MMoH 已完成的流水线划分与设备映射基础上，HR-MMoH 的异构重计算核心思想是：\emph{为每个流水级（或每台设备）根据其显存上限以及算力，独立配置重计算粒度与子模块集合}，
使显存压力大的流水级采用更激进的检查点以降低 OOM 风险，显存压力小的流水级采用较保守的配置以减少不必要的重计算开销，从而在满足显存约束的前提下最小化迭代时间并最大化 GPU 利用率。

\paragraph{重计算技术的定量分析。}
为定量比较不同重计算策略对显存占用的影响，本段在给定输入形状与数值格式下，对\emph{单层} Transformer 前向传播中需为反向传播保留的中间激活总量进行详细推导。
假设层输入张量形状为 $[B, S, H]$，其中 $B$ 为 batch size、$S$ 为序列长度、$H$ 为隐藏维度；
中间激活使用训练常用的 \texttt{bf16} 格式，每元素占 2 字节。采用常见的 Pre-LayerNorm 的 decoder-only 单层结构：
层输入 $\mathbf{x}$ 经 LayerNorm 后进入注意力子层，
残差连接后再经 LayerNorm 进入 MLP 子层（线性 $\to$ GELU $\to$ 线性 + Dropout），再次残差连接得到层输出。
记注意力头数为 $A$，MLP 层的中间维度设置为常用的设置，即 hidden size 的 4 倍，记为 $4H$
...
\emph{注意力子层}：线性投影输入 $2 B S H$、注意力 dropout mask $B S H$、Q/K/V 共享输入 $2 B S H$、$QK^{\top}$ 所需的 Q 与 K 共 $4 B S H$、softmax 输出 $2 B A S^2$、softmax dropout mask $B A S^2$、对 V 的注意力中 dropout 输出与 V 共 $2 B A S^2 + 2 B S H$，合计
\begin{equation}
  N_{\mathrm{attn}} = 11 B S H + 5 B A S^2.
\end{equation}
其中 $5 B A S^2$ 来自 $QK^{\top}$、softmax 及 dropout、attention-over-V 等，随序列长度平方增长。
...
\emph{MLP 子层}（中间维度 $4H$）：两个线性层各自保存输入（$2 B S H$ 与 $4H$ 维张量 $4 B S H$）、GELU 需保存其输入、dropout 需保存 mask，合计\cite{korthikanti2023activation}
\begin{equation}
...
\emph{两个 LayerNorm}（注意力前、MLP 前）各保存输入 $2 B S H$，共 $4 B S H$。于是，不采用重计算时单层需保存的中间激活总量为
\begin{equation}
  N_{\mathrm{None}} = N_{\mathrm{attn}} + N_{\mathrm{mlp}} + 4 B S H = 34 B S H + 5 B A S^2.
\end{equation}
其中 $5 B A S^2$ 项随序列长度平方增长，是长序列场景下激活显存的主要来源。

\textbf{（2）选择性重计算（\texttt{selective}）。} 仅对指定子模块做重计算：前向只保存该子模块的\emph{输入}，反向传播时在子模块内部重新前向传播再反传，其它未开启重计算的子模块仍按 \texttt{None} 情形完整保存中间激活。
以仅对 \texttt{core\_attn} 开启 selective 为例，注意力子层在 \texttt{None} 情形下需要 $N_{\mathrm{attn}} = 11 B S H + 5 B A S^2$ 个激活元素；
对 \texttt{core\_attn} 做 selective 后，注意力\emph{内部}的中间激活（$QK^{\top}$、softmax 及其 dropout mask 矩阵、attention-over-$V$ 等）都不再常驻，
只保留该子模块的输入张量（层输入和 LN 输出）以便在反向传播重新计算注意力块本身；
MLP 子层与两个 LayerNorm 的激活仍按照 $N_{\mathrm{mlp}}$ 与 $4 B S H$ 的代价保存，因此整层的线性部分从 $34 B S H + 5 B A S^2$ 约减少为 $23 B S H$。

若在此基础上进一步对 \texttt{mlp} 开启 selective，则 MLP 内部的 $19 B S H$ 激活也改为在反向时重算，其常驻部分被压缩为 “MLP 输入边界” ，此时需要保存的中间激活总量进一步降低为 $6 B S H$。
...
与 \texttt{None} 相比，选择重计算策略去掉了注意力主体与 MLP 主体中的大部分常驻中间激活，仅保留子模块边界的输入张量，显存由 $O(B S H + B A S^2)$ 降为 $O(B S H)$。
...
表~\ref{tab:chap4-activation} 汇总了不采用重计算、选择性重计算与整层重计算时单层需保存的中间激活总量。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{2.5cm}p{4.2cm}p{5.8cm}@{}}
    ...
    \hdashline
    \texttt{selective} [\texttt{core\_attn}, \texttt{mlp}, \texttt{layernorm}] & $4 B S H$ & 三者均重算，仅子模块边界输入常驻 \\
    \hdashline
