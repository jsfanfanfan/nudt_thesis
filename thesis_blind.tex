%% !TEX program = xelatex
%% 参考文献：本文件主名为 thesis_blind，BibTeX 须对 thesis_blind.aux 运行（生成 thesis_blind.bbl），
%% 不能只用 xelatex、也不能对 thesis 跑 bibtex。推荐：latexmk -xelatex thesis_blind.tex
%%
...
%
\documentclass[master,ttf,anon,twoside,prof]{nudtpaper}
\usepackage{mynudt}
...
\frontmatter
\makeatletter
\renewcommand{\cleardoublepage}{\cleardoublepagewithhead}
\makeatother
\cleardoublepage
\tableofcontents
\cleardoublepage
...
\input{data/resume_blind}    %在学期间成果
% \input{data/reviewinfo} %公开评阅信息
% \input{data/defensecommittee} %答辩委员会
